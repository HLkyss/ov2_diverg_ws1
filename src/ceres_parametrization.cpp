...
//    Eigen::Map<const Eigen::Vector4d> lcalib(parameters[0]);
    ...
    // [tx, ty, tz, qw, qx, qy, qz]
    Eigen::Map<const Eigen::Vector3d> twanch(parameters[1]);//锚定关键帧的位姿 右目到世界
    Eigen::Map<const Eigen::Quaterniond> qwanch(parameters[1]+3);

    Eigen::Map<const Eigen::Vector3d> twc(parameters[2]);//当前关键帧的位姿 右目到世界
    Eigen::Map<const Eigen::Quaterniond> qwc(parameters[2]+3);
    ...
    Sophus::SE3d Twanch(qwanch,twanch);//锚定关键帧的位姿 右目到世界
//    Twanch = Twanch*Tlr;//转换成右目位姿（左目到世界*右到左）
    Sophus::SE3d Twc(qwc,twc);//当前关键帧的位姿 右目到世界
//    Twc = Twc*Tlr;//转换成右目位姿（左目到世界*右到左）
    Sophus::SE3d Tcw = Twc.inverse();

    // [1/z_anch]
    const double zanch = 1. / parameters[3][0];//右目深度
    ...
    Eigen::Vector3d ranchpt = zanch * invK_r * ranchpx_;
//    Eigen::Vector3d wpt = Twanch * anchpt;
...
//            J_lcalib.setZero();
        ...
        if(jacobians[3] != NULL)
        {
            ...
            Eigen::Vector3d J_lambda = -1. * zanch * Twanch.rotationMatrix() * ranchpt;
...
//        Eigen::Matrix<double,2,3> J_rRcw;
        ...
        {
//            J_rRcw.noalias() = J_rcam * Trl.rotationMatrix();
            J_lRcw.noalias() = J_lcam * Trl.rotationMatrix();//
        }
...
//            J_invpt.noalias() = J_rRcw * J_lambda;
            Eigen::Map<Eigen::Vector2d> J_invpt(jacobians[3]);
            Eigen::Vector3d J_lambda = -1. * zanch * ranchpt;//与逆深度相关的梯度（即影响重投影误差的梯度）
                                                         ...
                                                         double* residuals, double** jacobians) const
{
    ...
    // [tx, ty, tz, qw, qx, qy, qz]
    Eigen::Map<const Eigen::Vector3d> twanch(parameters[2]);//锚定关键帧的位姿 右目到世界
    Eigen::Map<const Eigen::Quaterniond> qwanch(parameters[2]+3);

    Eigen::Map<const Eigen::Vector3d> twc(parameters[3]);//当前关键帧的位姿 右目到世界
    Eigen::Map<const Eigen::Quaterniond> qwc(parameters[3]+3);
