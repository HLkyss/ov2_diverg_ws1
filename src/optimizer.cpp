...
//    std::map<int,int> map_covkfs = newframe.getCovisibleKfMap();
    std::map<int,int> map_covkfs_l = newframe_l.getCovisibleKfMap();
...
//    int nmaxkfid = map_covkfs.rbegin()->first;
    ...
    ///全用左目的信息
    for( auto it = map_covkfs_l.rbegin() ; it != map_covkfs_l.rend() ; it++ ) {
        ...
        ceres::LocalParameterization *local_parameterization = new SE3LeftParameterization(); // 左右目用同一个?
        ...
        // todo stage2 添加一个 约束，来确保左目和右目位姿之间的关系保持不变
        if (map_id_posespar_r_.find(kfid) != map_id_posespar_r_.end()) {
            // 创建约束，确保左目和右目位姿相等
//            DirectLeftSE3::PoseEqualityConstraint* pose_constraint = new DirectLeftSE3::PoseEqualityConstraint();
//            problem.AddResidualBlock(pose_constraint, nullptr,
//                                     rlextrinpose.values(),
//                                     map_id_posespar_l_.at(kfid).values(),//左目位姿(左目到世界)
//                                     map_id_posespar_r_.at(kfid).values());//右目位姿(右目到世界)

            // 创建一个 AutoDiffCostFunction，自动求导
//            ceres::CostFunction* cost_function =
//                    new ceres::AutoDiffCostFunction<PoseEqualityConstraint, 7, 7>(
//                            new PoseEqualityConstraint());
//            problem.AddResidualBlock(cost_function, nullptr,
//                                     rlextrinpose.values(),  // 常量外参
//                                     map_id_posespar_l_.at(kfid).values(), // 待优化的左相机位姿
//                                     map_id_posespar_r_.at(kfid).values()); // 待优化的右相机位姿

//            ceres::CostFunction* cost_function0 =
//                    new ceres::AutoDiffCostFunction<PoseEqualityConstraint2, 7, 7,7,7>(
//                            new PoseEqualityConstraint2());
//            problem.AddResidualBlock(cost_function0, nullptr,
//                                     rlextrinpose.values(),  // 常量外参
//                                     map_id_posespar_l_.at(kfid).values(), // 待优化的左相机位姿
//                                     map_id_posespar_r_.at(kfid).values()); // 待优化的右相机位姿
...
//                    new ceres::AutoDiffCostFunction<CostFunctor, 3, 1, 1, 1>(new CostFunctor);
//            problem.AddResidualBlock(cost_function2, nullptr, &x, &y, &z);
//
//            double a,b,c = 0.23;
//            ceres::CostFunction* cost_function3 =
//                    new ceres::AutoDiffCostFunction<CostFunctor2, 3, 3>(new CostFunctor2);
//            problem.AddResidualBlock(cost_function3, nullptr, &a, &b, &c);
        }

        // For those to optimize, get their 3D MPs
        // for the others, set them as constant
        if( covscore >= nmincovscore && !all_cst && kfid > 0 ) {
            set_kfids2opt_l.insert(kfid);
            for( const auto &kp : pkf->getKeypoints3d() ) {
                set_lmids2opt_l.insert(kp.lmid_);
            }
        } else {
            set_cstkfids_l.insert(kfid);
    ...
    ///全用右目的信息
    for( auto it = map_covkfs_r.rbegin() ; it != map_covkfs_r.rend() ; it++ ) {

        int kfid = it->first;
        int covscore = it->second;

        if( kfid > newframe_r.kfid_ ) {
            covscore = newframe_r.nbkps_;
        }

        auto pkf = pmap_r_->getKeyframe(kfid);
        ...
        map_id_posespar_r_.emplace(kfid, PoseParametersBlock(kfid, pkf->getTwc()*Tlr));// 右目位姿 右目到世界（左目到世界*右目到左目）

        ceres::LocalParameterization *local_parameterization = new SE3LeftParameterization();// 左右目用同一个?

        problem.AddParameterBlock(map_id_posespar_r_.at(kfid).values(), 7, local_parameterization);// 右目位姿添加到问题中
        ordering->AddElementToGroup(map_id_posespar_r_.at(kfid).values(), 1);

        // todo stage2 添加一个 约束，来确保左目和右目位姿之间的关系保持不变
        if (map_id_posespar_l_.find(kfid) != map_id_posespar_l_.end()) {
            // 创建约束，确保左目和右目位姿相等
            ceres::CostFunction* cost_function0 =
                    new ceres::AutoDiffCostFunction<PoseEqualityConstraint2, 7, 7,7,7>(
                            new PoseEqualityConstraint2());
        ...
        // for the others, set them as constant
        if( covscore >= nmincovscore && !all_cst && kfid > 0 ) {
            set_kfids2opt_r.insert(kfid);
            for( const auto &kp : pkf->getKeypoints3d() ) {
                set_lmids2opt_r.insert(kp.lmid_);
            }
        } else {
            set_cstkfids_r.insert(kfid);
            problem.SetParameterBlockConstant(map_id_posespar_r_.at(kfid).values());
            all_cst = true;
        }
    ...
    for( const auto &lmid : set_lmids2opt_r ) {
        ...
        for( const auto &kfid : plm->getKfObsSet() ) {
            ...
            if( pkfit == map_local_pkfs_r.end() )
            {
                ...
                problem.AddParameterBlock(map_id_posespar_r_.at(kfid).values(), 7, local_parameterization);//补充
                ordering->AddElementToGroup(map_id_posespar_r_.at(kfid).values(), 1);
            ...
            if( kp.is_stereo_ ) {
                if( pslamstate_->buse_inv_depth_ ) {
                    ...
                    rid = problem.AddResidualBlock(
                            ...
                            map_id_posespar_r_.at(kfanchid).values(),//锚定关键帧的位姿  右目位姿（右目到世界）
                            map_id_posespar_r_.at(kfid).values(),//当前关键帧的位姿
                    ...
                    rid = problem.AddResidualBlock(
                            f, loss_function,
                            calibpar.values(), rightcalibpar.values(),
                            map_id_posespar_r_.at(kfanchid).values(),//锚定关键帧的位姿  右目原本存的也是左目位姿（左目到世界），可能要转换成右目位姿（左目到世界*右到左）
                            map_id_posespar_r_.at(kfid).values(),//当前关键帧的位姿
                            rlextrinpose.values(),
                            map_id_invptspar_r_.at(lmid).values()//地图点的逆深度参数
                    );
                    ...
                    rid = problem.AddResidualBlock(
                            f, loss_function,
                            calibpar.values(),
                            map_id_posespar_r_.at(kfid).values(),
                ...
                if( pslamstate_->buse_inv_depth_ ) {
                    ...
                    rid = problem.AddResidualBlock(
                            ...
                            map_id_posespar_r_.at(kfanchid).values(),//7
                            map_id_posespar_r_.at(kfid).values(),//7
    ...
    if( nbcstkfs_l < nmincstkfs ) {
        for( auto it = map_local_pkfs_l.begin() ; nbcstkfs_l < nmincstkfs && it != map_local_pkfs_l.end() ; it++ )
        {
            problem.SetParameterBlockConstant(map_id_posespar_l_.at(it->first).values());
            set_cstkfids_l.insert(it->first);
            nbcstkfs_l++;
        }
    }
    if( nbcstkfs_r < nmincstkfs ) {
        for( auto it = map_local_pkfs_r.begin() ; nbcstkfs_r < nmincstkfs && it != map_local_pkfs_r.end() ; it++ )
        {
            problem.SetParameterBlockConstant(map_id_posespar_r_.at(it->first).values());
            set_cstkfids_r.insert(it->first);
            nbcstkfs_r++;
        }
    }
    ...
    size_t nbkfs2opt_l = nbkfstot_l - nbcstkfs_l;// 参与优化的关键帧数量
    size_t nbkfs2opt_r = nbkfstot_r - nbcstkfs_r;
    size_t nblms2opt_l = map_local_plms_l.size();// 参与优化的地图点数量
    size_t nblms2opt_r = map_local_plms_r.size();

    if( pslamstate_->debug_ ) {
        std::cout << "\n\n >>> LocalBA problem setup!";
        std::cout << "\n >>> left Kfs added (opt / tot) : " << nbkfs2opt_l
            << " / " << nbkfstot_l;
        std::cout << "\n >>> right Kfs added (opt / tot) : " << nbkfs2opt_r
                  << " / " << nbkfstot_r;
        ...
        for( const auto &id_pkf : map_local_pkfs_l ) {
            std::cout << " KF #" << id_pkf.first << " (cst : " << set_cstkfids_l.count(id_pkf.first) << "), ";
        }
        std::cout << "\n\n >>> right KFs added : ";
        for( const auto &id_pkf : map_local_pkfs_r ) {
            std::cout << " KF #" << id_pkf.first << " (cst : " << set_cstkfids_r.count(id_pkf.first) << "), ";
        }
    ...
    options.max_solver_time_in_seconds = 0.6;//0.2
...
//        if( pslamstate_->debug_ )
    ...
    for( const auto &kfid_pkf : map_local_pkfs_r )    ///right
    {
        ...
        // auto optkfpose = map_id_posespar_.at(kfid);
        auto it = map_id_posespar_r_.find(kfid);
        if( it != map_id_posespar_r_.end() ) {
